...
static void test_fnv_tbl_init(fnv_tbl_t *tbl, size_t c);
...
static void test_fnv_get(void) {
    ...
    size_t c = sizeof(test_keys) / sizeof(char *);
...
static void test_fnv_out_notfound(void) {
    ...
    CU_ASSERT(fnv_out(tbl, "OUTA",   strlen("OUTA"))   == FNV_OUT_NOTFOUND);
    fnv_tbl_destroy(tbl);
}
...
static void test_fnv_minimul_cnt(void) {
    ...
    CU_ASSERT(fnv_get(tbl, "EI",   strlen("EI"))   == NULL);
    CU_ASSERT(fnv_get(tbl, "OTIR", strlen("OTIR")) == NULL);
    CU_ASSERT(fnv_get(tbl, "EXX",  strlen("EXX"))  == NULL);
